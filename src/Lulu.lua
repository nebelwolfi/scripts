...
local version = 0.17 -- REMEMBER: UPDATE .version FILE ASWELL FOR IN-GAME PUSH!
...
function LoadMenu()
    ...
    menu.growthsettings:addParam("autorper", "Auto R ally under health %", SCRIPT_PARAM_SLICE, 30, 1, 100, 0)
    menu.growthsettings:addParam("smartr","Use if x enemies hitable", SCRIPT_PARAM_SLICE, 3, 1, 5, 0)
    
    menu.growthsettings:addSubMenu("Save Priority","custompriority")
    Boot()
...
function Boot()
  ...
  menu.growthsettings.custompriority:addParam("info1"," >> Highest priority to save: ", SCRIPT_PARAM_INFO, 1)
  alliedHeros = {}
  alliedHerosCount = 0
    for i = 1, heroManager.iCount do
    local hero = heroManager:GetHero(i)
    if hero.team == player.team then
      ...
      menu.growthsettings.custompriority:addParam("champ"..(alliedHerosCount-1),hero.charName,SCRIPT_PARAM_SLICE, 1, 1, 5, 0)
    end
  end
  BootDone = true
end
...
function SmartR()
  if RReady and menu.growthsettings.user then
    if player.health < (player.maxHealth*(menu.growthsettings.autorper*0.01)) and countEnemy(player, 200) >= 1 then
      CastSpell(_R, player)
    end
    for _, i in pairs (allies) do
      if i.health < (i.maxHealth*(menu.growthsettings.autorper*0.01)) and countEnemy(i, 200) >= 1 then
        CastSpell(_R, i)
      end
    end
